...
from Stock_Prediction_Data_Stock import updateStockData, getStocksList
...
def get_single_stock_data(ticker, stock_folder):
    ...
    #df.rename(columns=dict(zip(COLUMNS, RENAME_COLUMNS)), inplace=True)
    if 'Adj Close' in df:
        close = 'Adj Close'
    else:
        close = 'Close'
...
def get_all_stocks_data():
    ...
    stocklist = getStocksList(share_dir, 100)
...
updateStockData([""], "1990-01-01", now, 100, True)
